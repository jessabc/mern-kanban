...
export default function Main({isMenuModalVisible, setIsMenuModalVisible}) {
  ...
  const onDragEnd = ({ source, destination }) => {
    ...
    const start = columns.find(column => column.columnName === source.droppableId)
    const end = columns.find(column => column.columnName === destination.droppableId)
    ...
    if (start === end) {
      ...
      // Update the state
      let updatedColumns = currentBoardData.columns.map(column => column.columnName === source.droppableId ? newStartCol : column)
